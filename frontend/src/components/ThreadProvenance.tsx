...
/** Hauteur de bloc fixe, dictée par le texte et non par un décompte : depuis que chaque article
 *  tire son propre fil, c'est le nombre de fils qui porte la quantité. Les blocs n'ont plus à être
 *  proportionnels — ils n'ont qu'à rester lisibles. */
const BLOCK_H = 54;
const GAP = 10;
/** Retrait des ancrages aux bords du bloc, pour qu'un fil ne parte jamais de l'arête. */
const ANCHOR_PAD = 7;
const UNPLACED = "__unplaced";

/** Le niveau de provenance est porté par la forme du trait, jamais par la seule couleur : la
 *  distinction entre un pays cité et un pays présumé doit survivre à un daltonisme comme à une
 *  impression en noir et blanc. La légende en donne l'échantillon, les blocs la redisent en mots. */
/** Motifs calibrés pour un trait épais à bouts ronds : le « pointillé » est un tiret de longueur
 *  nulle, que le bout rond transforme en pastille ronde. Les mêmes valeurs servent au fil et à son
 *  échantillon de légende — ils ne peuvent donc pas diverger. */
...
/** Position verticale d'un fil dans son bloc : réparti régulièrement, jamais collé aux arêtes. */
const anchor = (blockY: number, slot: number, of: number) =>
  blockY + ANCHOR_PAD + ((BLOCK_H - 2 * ANCHOR_PAD) * (slot + 0.5)) / of;

/** Répartit les fils d'un même bloc dans l'ordre du bloc opposé, pour limiter les croisements
 ...
 *
 *  Les deux colonnes sont deux dimensions distinctes que l'interface ne doit jamais confondre :
 *  à gauche le pays du média, à droite le pays de l'événement tel que `resolveLocation` le résout.
 *  Le pays d'un média ne rattache pas à lui seul un article — sans quoi une dépêche TASS sur le
 *  Yémen se lirait comme une actualité russe (cf. lib/geo.ts, docs/cadrage.md §11). C'est
 *  précisément l'écart entre les deux colonnes qui porte l'information : un fil couvert par une
 *  agence d'État étrangère ne se lit pas comme une couverture domestique.
 *
 *  Un fil par article, plutôt qu'un ruban épais par flux : la quantité se compte au lieu de se
 *  jauger, chaque trait reste rattachable à son article au survol, et le niveau de provenance se
 *  lit sur le trait lui-même — ce qu'un ruban dont seul le contour était tireté ne permettait pas. */
export function ThreadProvenance({ thread }: { thread: ThreadModel }) {
  const { left, right, strands, height } = useMemo(() => {
    ...
    const rightBlocks: Block[] = [...thread.coverage.byCountry.entries()]
      ...
      .map(([key, bucket]) => {
        ...
        // Le décompte d'abord, comme dans la colonne des médias : sans lui, la ventilation par
        // niveau était le seul chiffre du bloc et les deux colonnes ne se comparaient plus.
    ...
    // Colonnes centrées l'une par rapport à l'autre : sans ça, deux colonnes de hauteurs
    // différentes s'alignent en haut et les fils partent tous en biais vers le bas.
  ...
  return (
    <div className="pv-wrap">
          ...
          {strands.map((s) => {
            ...
            return (
              <path
                ...
                /* Points de contrôle croisés (58 puis 42) plutôt que tous deux au milieu : le fil
                   quitte et rejoint ses blocs bien à plat, et toute la courbure se concentre dans
                   une inflexion centrale nette — plus calme à l'œil qu'une bascule diffuse sur
                   toute la largeur. */
      ...
      {/* Les trois niveaux sont toujours affichés, y compris à zéro : c'est le décompte séparé qui
          fait l'information, et un niveau masqué parce que vide se lirait comme un niveau inexistant. */}
      <ul className="pv-key">
      ...
      <p className="note pv-note">
        Un fil par article : à gauche le pays du <strong>média</strong>, à droite celui de
